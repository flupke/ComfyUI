...
def load_state_dict(model, ckpt_path, device="cuda", strict=False, assign=True):
    if Path(ckpt_path).suffix == ".safetensors":
        state_dict = load_file(
            os.path.join(folder_paths.models_dir, "Step1x-Edit", ckpt_path), device
        )
    else:
...
class ImageGenerator:
    ...
    def prepare(self, prompt, img, ref_image, ref_image_raw):
        ...
        ref_img = rearrange(
            ref_image,
            "b c (ref_h ph) (ref_w pw) -> b (ref_h ref_w) (c ph pw)",
            ph=2,
            pw=2,
        )
    ...
    def generate_image(
        ...
        ref_images_raw, img_info = self.input_process_image(
            ref_images, img_size=size_level
        )
...
folder_paths.folder_names_and_paths["MLLM"] = (
    current_paths,
...
folder_paths.folder_names_and_paths["Step1x-Edit"] = (
    current_paths,
    folder_paths.supported_pt_extensions,
)
            ...
            "required": {
                ...
                "seed": (
                    "INT",
                    {
                        ...
                        "tooltip": "The random seed for generation.",
                    },
                ),
                "cfg": (
                    "FLOAT",
                ...
                "num_steps": (
                    "INT",
                    {
                        ...
                        "min": 0,
                        "max": 10000,
                        "tooltip": "The number of diffusion steps.",
                    },
                ),
                "step1x_edit_model": (folder_paths.get_filename_list("Step1x-Edit"),),
                "step1x_edit_model_vae": (
                    folder_paths.get_filename_list("Step1x-Edit"),
                ),
                ...
                "offload": (
                    ...
                    {
                        "default": False,
                        "tooltip": "Enable offloading the model to CPU.",
                    },
                ),
                "quantized": (
                    "BOOLEAN",
                    {"default": False, "tooltip": "Enable quantization of the dit."},
                ),
            }
        }
